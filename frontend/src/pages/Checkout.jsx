...
function Checkout() {
  ...
  const validate = () => {
    ...
    if (formData.paymentMethod === "tarjeta") {
      // El pago con tarjeta se procesa de forma segura con PayU en el Checkout.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post("/pedidos/payu-session", payload);
      const { payuFormUrl, payuParams } = response.data?.data || response.data;

      if (!payuFormUrl || !payuParams) {
        console.error("No se pudo obtener los parámetros de PayU:", response.data);
        showToast("Error interno creando la sesión de pago", "error");
        return;
      }

      showToast("Redirigiendo a PayU para completar el pago...", "success");

      // Crear y enviar el formulario a PayU automáticamente
      const form = document.createElement("form");
      form.method = "POST";
      form.action = payuFormUrl;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {formData.paymentMethod === 'tarjeta' && (
            <div className="rounded-xl border border-cuero/20 bg-cuero/5 p-4 text-sm text-cuero-dark">
              ...
              <p>
                Serás redirigido a Stripe para completar el pago. En ningún
