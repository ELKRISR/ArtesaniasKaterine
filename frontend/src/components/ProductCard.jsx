...
function ProductCard({ product }) {
   ...
   * Usa fallback si: no hay URL, o si la URL falló al cargar.
   */
  const imageSrc = (!product.imagen || imgError)
    ? IMAGEN_FALLBACK
    : product.imagen;
  ...
  return (
    <div className="card">
      ...
      {/* ── Nombre ────────────────────────────────────────── */}
      <h2 className="subheading mb-2">{product.nombre}</h2>

      {/* ── Descripción ───────────────────────────────────── */}
      <p className="text-body mb-4">{product.descripcion}</p>
